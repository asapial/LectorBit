...
| Local Whisper transcription           | Implemented                | Authorized media lookup, sidecar validation, persistent jobs, restart recovery, versioned transcript segments                        | Add language/model choice, cancellation, transcript correction, confidence display, and performance presets                                   |
| Keyword transcript search             | Implemented                | SQLite FTS5 over media names, transcripts, and annotations; timestamp results                                                        | Add filters, result grouping, hybrid semantic retrieval, and query suggestions                                                                |
...
| Natural-language planning constraints | Implemented                | Model output is normalized into typed fields and Rust validates it                                                                   | Add change preview/diff, clearer rejected-field feedback, and evaluation fixtures                                                             |
| AI prerequisite/order suggestion      | Implemented                | Limited candidate envelope, redacted paths, optional grounded summaries, deterministic feasibility boundary                          | Consolidate provider fallback logic and record request provenance without sensitive content                                                   |
| Lecture understanding                 | Implemented                | Persistent background job, input hash, versioned artifact, prompt/schema version, canonical timestamp hydration, citation validation | Add cancellation/retry UI, stale-artifact indication, partial progress, and transcript correction loop                                        |
...
| Study material generation             | Implemented                | Four item types, transcript citations, normalized storage                                                                            | Add deduplication scoring, difficulty controls, editable items, and quality feedback                                                          |
| Review scheduling                     | Implemented, deterministic | Append-only attempts and SM-2-style local due-date calculation                                                                       | Add a dedicated due-review queue, mastery summaries, lapse targeting, and plan integration                                                    |
...
| AI jobs and recovery                  | Partially unified          | Transcription and lecture understanding recover after restart                                                                        | Use one job taxonomy, common cancellation/retry states, priority, backoff, concurrency, and status UI                                         |
...
| AI evaluation and observability       | Early                      | Strong DTO validation, prompt-injection framing, and many deterministic tests                                                        | Add golden datasets, citation precision, unsupported-claim rate, latency, provider/model provenance, token/cost budgets, and regression gates |
| AI discovery UX                       | Improved in this audit     | AI Studio now centralizes readiness, workflows, trust boundaries, and honest roadmap labels                                          | Add live artifact counts, job controls, model recommendations, onboarding, and global command access                                          |
